...
function comSegmentos(serie: ReturnType<typeof makeSerie>, segmentos: { cargaKg: number | null; repeticoes: number | null }[]) {
  return {
    ...serie,
    segmentos: segmentos.map((s, i) => ({
      id: `${serie.id}_seg${i + 2}`,
      serieId: serie.id,
      ordem: i + 2,
    ...
    it('e1RM/melhorSerie ignoram os degraus (so a serie-mae conta)', () => {
      const vm = buildSessaoResumoViewModel(
        makeDetalhe({
          exercicios: [
            makeExercicioComSeries('se_1', [
              comSegmentos(makeSerie('s1', 'valida', 60, 8), [{ cargaKg: 90, repeticoes: 10 }]),
            ]),
          ],
        })
      );

      expect(vm.exercicios[0].melhorSerie).toBe('60kg × 8');
    });
  });
