...
export class SQLiteTreinoExercicioRepository implements TreinoExercicioRepository {
  ...
  async save(treinoExercicio: TreinoExercicio): Promise<void> {
    ...
    await this.database.run(
      `
        INSERT OR REPLACE INTO treino_exercicios (id, treino_id, exercicio_id, ordem)
        VALUES (?, ?, ?, ?)
      `,
      [p.id, p.treinoId, p.exercicioId, p.ordem]
    );
  }

  async listByTreinoId(treinoId: string): Promise<TreinoExercicio[]> {
    const rows = await this.database.getAll<TreinoExercicioRow>(
      'SELECT id, treino_id, exercicio_id, ordem FROM treino_exercicios WHERE treino_id = ? ORDER BY ordem ASC',
      [treinoId]
    );
  ...
  async findById(id: string): Promise<TreinoExercicio | null> {
    const row = await this.database.getFirst<TreinoExercicioRow>(
      'SELECT id, treino_id, exercicio_id, ordem FROM treino_exercicios WHERE id = ? LIMIT 1',
      [id]
    );
  ...
  async findByTreinoIdAndExercicioId(
    ...
    const row = await this.database.getFirst<TreinoExercicioRow>(
      'SELECT id, treino_id, exercicio_id, ordem FROM treino_exercicios WHERE treino_id = ? AND exercicio_id = ? LIMIT 1',
