...
describe('GetTreinoEvolucaoUseCase', () => {
  it('returns evolucao for given treinoId', async () => {
    const data = [{ exercicioId: 'ex1' }] as never;
    const repo = { getEvolucaoExercicios: vi.fn().mockResolvedValue(data) } as never;
  ...
  it('propagates errors from repository', async () => {
    const repo = { getEvolucaoExercicios: vi.fn().mockRejectedValue(new Error('db error')) } as never;
    await expect(
